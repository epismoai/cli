...
func caseCommands() []*command {
	...
	get := publicApiOperation("case get", "read saved case context before continuing; pass the Case code (for example PANDA-317) or UUID. Check goals, constraints, decisions, artifacts, and next steps. Public cases return published content without login; this does not restore an agent session", []string{"case-id"}, http.MethodGet, func(i invocation) string { return "/v1/cases/" + escaped(i.positional(0)) }, requestNone)
	list := apiOperation("case list", "find collaborator-accessible cases to resume, most recently updated first; match the intended goal before case get, and ask if ambiguous", nil, http.MethodGet, staticEndpoint("/v1/cases"), requestQuery, false, append(pagingOptions(), str("--assigned-to", "assignedTo", "only cases assigned to this account"), choice("--status", "status", "case status", "open", "completed", "cancelled"))...)
	popular := publicApiOperation("case popular", "rank public cases for discovery", nil, http.MethodGet, staticEndpoint("/v1/cases/popular"), requestQuery, append(pagingOptions(), str("--playbook-id", "playbookId", "only cases started from this playbook"))...)
	...
	close := lockedMutation("case close", "close a case", "case-id", http.MethodPost, func(i invocation) string { return "/v1/cases/" + escaped(i.positional(0)) + "/close" }, choice("--status", "status", "case status", "completed", "cancelled"), array("--records", "records", "JSON array of final records"))
