...
func caseCommands() []*command {
	...
	review := apiOperation("case review", "queue an Epismo AI review of this open Case's shared Records and Tasks and return immediately. Any Case editor may request it. Poll case record list with kinds=review and origins=system; replay the same idempotency key after completion to receive the REVIEW Record. Distinct from a human- or agent-authored review Record and from an APPROVAL Task. Credits are charged to the Case billing account captured at start", []string{"case-id"}, http.MethodPost, func(i invocation) string { return "/v1/cases/" + escaped(i.positional(0)) + "/review" }, requestBody, true)
